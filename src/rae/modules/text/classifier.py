        ...
        with torch.no_grad():
            if self.anchor_encodings is None:
                ...
                for anchors_batch in chunk_iterable(self.metadata.anchor_samples, 128):
                    anchors_batch = to_device(self.text_encoder.collate_hf_fn(batch=anchors_batch), device=device)
                    anchor_encodings = self.call_transformer(
                        encodings=anchors_batch["encodings"], sample_ids=anchors_batch["index"]
                    )
                    # (batch, hidden), (64, hidden),
...
class JointTextClassifier(nn.Module):
    ...
    def __init__(
        self,
        metadata: MetaData,
        text_encoders: Sequence[DictConfig],
        ...
        **kwargs,
        ...
        self.text_encoders: Sequence = nn.ModuleList([instantiate(text_encoder) for text_encoder in text_encoders])

    def encode(self, batch, device: Device):
        _ = [text_encoder(batch) for text_encoder in self.text_encoders]
