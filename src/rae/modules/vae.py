...
class VAE(nn.Module):
    def __init__(self, metadata: MetaData, hidden_channels: int, latent_dim: int):
        ...
        self.decoder = Decoder(
            hidden_channels=hidden_channels, latent_dim=2 if self.anchors is None else self.anchors.shape[0]
        )
    ...
    def forward(self, x):
        ...
        latent, latent_mu, latent_logvar = self.embed(x)

        if self.anchors is not None:
            latent = torch.einsum("bi, ji -> bj", (latent, anchors_latent))

        #     print(latent.shape, anchors_latent.shape, relative_latents.shape)
