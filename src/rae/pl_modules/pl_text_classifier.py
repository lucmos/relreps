...
def _build_metrics(split: str, num_classes: int) -> nn.ModuleDict:
    metrics: nn.ModuleDict = nn.ModuleDict(
        {
            f"f1/{split}/{average if average != 'none' else ''}": F1Score(num_classes=num_classes, average=average)
            for average in ("macro", "micro", "weighted", "none")
        }
    )
    for average in ("macro", "micro", "weighted", "none"):
        metrics[f"precision/{split}/{average if average != 'none' else ''}"] = Precision(
            num_classes=num_classes, average=average
        )

    for average in ("macro", "micro", "weighted", "none"):
        metrics[f"recall/{split}/{average if average != 'none' else ''}"] = Recall(
            num_classes=num_classes, average=average
        )

    for average in ("macro", "micro", "weighted", "none"):
        metrics[f"accuracy/{split}/{average if average != 'none' else ''}"] = Accuracy(
            num_classes=num_classes, average=average
        )
...
class LightningTextClassifier(AbstractLightningModule):
    ...
    def __init__(self, metadata: Optional[MetaData] = None, *args, **kwargs) -> None:
        ...
        self.train_metrics: nn.ModuleDict = _build_metrics(split="train", num_classes=num_classes)
        self.val_metrics: nn.ModuleDict = _build_metrics(split="val", num_classes=num_classes)
    ...
    def step(self, batch, batch_index: int, stage: Stage) -> Mapping[str, Any]:
        ...
        for metric_name, metric in metrics.items():
            metric_out = metric(out[Output.INT_PREDICTIONS], batch["target"].flatten())
            # If this metric had no previous aggregation...
            if metric_name.endswith("/"):
                metric_out = {f"{metric_name}{label}": score for label, score in zip(self.hparams.labels, metric_out)}
    ...
    def validation_epoch_end(self, outputs: List[Dict[str, Any]]) -> None:
        ...
        if self.anchor_samples is not None:
            anchors_num = self.anchor_samples.shape[0]
            anchors_out = self(self.anchor_samples)
        ...
        aggregate(
            validation_aggregation,
            sample_index=self.metadata.anchors_idxs
